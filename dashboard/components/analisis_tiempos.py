...
def to_excel(df: pd.DataFrame):
    ...
    for column in df:
        ...
        writer.sheets['Datos'].set_column(df.columns.get_loc(column), df.columns.get_loc(column), column_width)
    writer.close()
...
def mostrar_analisis_tiempos(df_filtrado):
    ...
    st.download_button("📥 Descargar Tabla Comparativa", to_excel(df_final), f"comparativa_tiempos_{pd.Timestamp.now().strftime('%Y%m%d')}.xlsx")
    ...
    # Traducción de las etiquetas de Plotly que aparecen al pasar el ratón
    fig.update_traces(
        hovertemplate="<b>Tipo de Operación:</b> %{x}<br>" +
                      ...
                      "<extra></extra>", # Esto oculta la etiqueta "trace"
        customdata=np.stack((
    ...
    st.download_button("📥 Descargar Rendimiento por Operativo", to_excel(df_operativo_tiempos), f"rendimiento_operativo_{pd.Timestamp.now().strftime('%Y%m%d')}.xlsx")